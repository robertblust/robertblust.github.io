...
- `mental-model-de.pdf` / `mental-model-en.pdf` – the exported PDF fallback

The PDF and share-card exporters (`export-pdf.mjs`, `export-og.mjs`) live at the repository root and build both decks; there is no per-deck copy.
...
npm run pdf        # → both decks, both languages: <slug>-de.pdf and <slug>-en.pdf
```
