...
Robert Blust's profile page and its talks. Self-contained, no bundler, no external assets. Working conventions and the traps that break this site silently are in `AGENTS.md`.
...
npm run pdf            # regenerate every deck's PDF fallbacks
...
./tts/generate.py               # narration: generate what changed, every deck
```
