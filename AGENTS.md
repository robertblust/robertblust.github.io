...
- **A deck links shared files, and is no longer required to open from `file://`.** It used
  ...
  generated by `@robertblust/design` and copied here by `npm run design`, are what a talk
  now presents from, and what gets sent to anyone is still the PDF, not the folder. A talk
  is normally read on the site, served.
...
What used to be ten fenced blocks are mostly whole files now: `tokens.css`, `page.css` and `page.js` carry what `design tokens`, `header contract`, `title contract`, `prose reset`, `prose footer`, `language`, `theme` and `nav fit` used to fence into every prose page; `deck.css` and `deck.js` carry `deck transport`, `deck lockup`, `deck fit` and `deck runtime` — the last with a `language` fence nested inside it — for a deck. A page still carries a handful of fences a file cannot replace: `theme boot`, the one script that has to run inline and synchronously in `<head>`, before the first paint, so it is the sole copy left in every page's own source; and, on the pages that draw the model, `stage contract`, `model card` and `surfaces lineage`, which are this site's own glue to what each page draws rather than shared boilerplate.
...
### The deck's chrome is generated files now, not four fences

The deck's whole chrome and runtime used to be four generated fences — `deck transport`, `deck lockup`, `deck fit` and `deck runtime`, the last carrying a nested `language` fence of its own — the way the deck footer's hand-maintained copy was retired for fences in an earlier plan. `deck.css` and `deck.js` carry all four now. A deck links both, states its own `TALK` and `UI` — the talk's title and description in both languages, and the transport's strings built from them — and declares `window.rbDeck = { talk: TALK, ui: UI }` before `<script src="../../deck.js" defer>`, the same shape a prose page declares `window.rbPage` before `page.js`. `theme` and `deck fit` ship inside `deck.js` alongside `deck runtime`; only `theme boot` stays a fence, for the reason above.

What the files cover is a **contract, not a look**: the lockup goes to the landing page, the person to `blust.ch`, the third link to the talks index, and none of them opens in a new tab. That contract still holds; `design:check` enforces it, comparing the generated files' bytes against the pinned release — the same guarantee the old `deck footer · vN` marker and its retired `footerVersion` check reached for by hand, in three repositories, and never as strongly.

`verify/design.mjs` now lives in `@robertblust/design`, alongside the shared page checks — edited there, released as a tag, and taken here by re-pinning that tag in `package.json`. `verify/check.mjs` imports it by package specifier, `@robertblust/design/verify/design`; a `verify/design.mjs` created in this repository is never resolved by that import and would be silently ignored — the suite would still report green, having run the pinned release's code instead of the one just edited.
...
**The head contract is a third copy**, shared with `guestgraph.io` and `companygraph.io` and carrying no `· vN` tripwire, unlike the tokens and the rest of what `@robertblust/design` generates — the files above and `theme boot`, `stage contract`, `model card` and `surfaces lineage` below. Port changes by hand to all three.
